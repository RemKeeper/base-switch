...
// ModelFetcher 模型获取器函数类型：根据 API 类型和连接配置获取模型列表
type ModelFetcher func(apiType, baseURL, apiKey, proxyURL string) ([]string, error)
...
func (m *Manager) AutoDiscoverModels(fetcher ModelFetcher) (int, error) {
	...
	for _, p := range providers {
		models, err := fetcher(p.APIType, p.BaseURL, p.APIKey, p.ProxyURL)
		if err != nil {
...
func (m *Manager) AddProvider(name, apiType, baseURL, apiKey, proxyURL string, models []string, enabled bool, fetcher ModelFetcher) error {
	if len(models) == 0 && fetcher != nil {
		discoveredModels, err := fetcher(apiType, baseURL, apiKey, proxyURL)
		if err != nil {
	...
	return m.store.InsertProvider(name, apiType, baseURL, apiKey, proxyURL, models, enabled)
}

// UpdateProvider 更新 Provider
func (m *Manager) UpdateProvider(name, apiType, baseURL, apiKey, proxyURL string, models []string, enabled bool) error {
	return m.store.UpdateProvider(name, apiType, baseURL, apiKey, proxyURL, models, enabled)
}
